...
| Proof points | "Every invocation logs four pieces of evidence - iq_hits, memory_injected, tools_called, inference_usage - so the replay log proves agents worked, not just that a model answered." | `CHAPTER_EXECUTED` events in the replay log, `tools/demo_smoke_test.py` evidence path |
...
| 9 | Four proof points per invocation | DONE (all paths, tested) | `CHAPTER_EXECUTED` payloads, smoke test `check_evidence_path` | "Every run logs iq_hits, memory_injected, tools_called, inference_usage - the smoke test fails the build if any chapter ships without evidence." |
